...
class HistoricMinDataHandler(DataHandler):
    ...
    def _open_convert_csv_files(self):
        ...
        for s in self.symbol_list:
            ...
            self.latest_symbol_data[s] = deque(maxlen=600) # list 크기 600개로 고정 ! 추후 수정 ㄱㄱ
    ...
    def update_bars(self, i):
        ...
        for s in self.symbol_list:
            ...
            try:
                ret = self.symbol_data[s].iloc[i,:]
            except IndexError:
                self.continue_backtest = False
            else:
                if ret is not None:
                    ret_tuple = (ret.name, ret)
                    self.latest_symbol_data[s].append(ret_tuple)
